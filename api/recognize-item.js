...
- Если на вещи есть принт/узор (горошек, цветы, клетка, полоска, леопардовый, зебра и т.п.) — ОБЯЗАТЕЛЬНО заполни поле "patternColorIds" списком ВСЕХ цветов узора (например для леопардового принта — обычно beige/camel как фон плюс brown и black как пятна). Базовый colorId в этом случае ставь как наиболее заметный/доминирующий из этих же цветов — никогда не оставляй patternColorIds пустым, если узор присутствует.

...
export default function AuthScreen({ onAuthenticated }) {
 ...
 async function handleSubmit(e) {
   ...
   try {
     if (mode === "login") {
       ...
       if (result.error) {
         setError(getSafeErrorText(result.error));
       } else {
 ...
 function getSafeErrorText(supaError) {
   let text = "Произошла ошибка. Попробуйте ещё раз.";
   if (supaError && supaError.message) {
     text = String(supaError.message);
   }
...
<p className="text-sm mb-7" style={{ color: "#8a7d6a" }}>
       ...
       {confirmMessage && (
<div className="mb-4 rounded-xl px-4 py-3 text-sm" style={{ backgroundColor: "#dce8d4", color: "#3a4f2f" }}>
           {confirmMessage}
</div>
       )}
       {error && (
<div className="mb-4 rounded-xl px-4 py-3 text-sm" style={{ backgroundColor: "#f0e6d4", color: "#6b5a3f" }}>
           {error}
</div>
       )}
